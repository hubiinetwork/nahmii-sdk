...
function hashObject(obj, propertyNameGlobs, prevHashValue) {
    ...
    if (isArrayPropertyGroup && expandedPropertyNames.length === 0) 
        return hash(0);
    ...
    const propertyValues = expandedPropertyNames.map(path => {
        ...
        if (Array.isArray(path)) {
            if (isArrayPropertyGroup) {
                value = hashObject(obj, path, prevHashValue || 0);
                prevHashValue = value;
            } else {
...
function expandPropertyNameGlobs(object, globPatterns) {
    ...
    if (globPatterns[0].indexOf('[]') !== -1) {
        const arrayPropName = globPatterns[0].substring(0, globPatterns[0].indexOf('[]') - 1);
        const arrayProp = getNestedProperty(object, arrayPropName);
        if (!arrayProp)
            throw new GlobExpandError(`No property matching "${globPatterns[0]}"`, globPatterns[0]);

        if (!(arrayProp instanceof Array))
            throw new GlobExpandError(`Property matching "${globPatterns[0]}" is not an array`, globPatterns[0]);
