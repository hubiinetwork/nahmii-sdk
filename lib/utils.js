...
function expandPropertyNameGlobs(object, globPatterns) {
    ...
    if (indexOfArrayGroup !== -1) {
        const arrayPropName = arrayGroupPattern.substring(0, indexOfArrayGroup - 1);
        const arrayProp = getNestedProperty(object, arrayPropName);
        if (!arrayProp)
            throw new GlobExpandError(`No property matching "${arrayGroupPattern}"`, arrayGroupPattern);

        if (!(arrayProp instanceof Array))
            throw new GlobExpandError(`Property matching "${arrayGroupPattern}" is not an array`, arrayGroupPattern);
