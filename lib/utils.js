...
/**
 * Checks that input is a valid hexadecimal string address
 * @param addr - representation of address
 * @returns {Boolean} true - valid hexadecimal address string with or without '0x' prefix. false - not a string or an invalid string
 */
function isValidAddressString(str) {
...
function sign(message, privateKey) {
    ...
    let pk = Buffer.from(strip0x(privateKey), 'hex');
    const ethMessageBuf = Buffer.from(strip0x(ethMessage), 'hex');
...
function recover(message, signature) {
    const messageBuffer = Buffer.from(strip0x(message), 'hex');
    const v = parseInt(signature.v);
    const r = Buffer.from(strip0x(signature.r), 'hex');
    const s = Buffer.from(strip0x(signature.s), 'hex');
