 ...
 * to the function.
 * @param {...List} balances - Balances to sum
 * @returns {List} A single balance
 */
function sumBalances(...balances) {
    return balances.reduce((sum, cur) => {
        cur.forEach((bal) => {
            const balIndex = sum.findIndex(b => JSON.stringify(b.currency) === JSON.stringify(bal.currency));
            if (balIndex === -1) {
                sum.push(bal);
            }
            else {
                sum[balIndex] = {
                    ...sum[balIndex],
                    amount: BigNumber(sum[balIndex].amount).plus(bal.amount).toString(),
                    decimalAmount: BigNumber(sum[balIndex].decimalAmount).plus(bal.decimalAmount).toFixed()
                };
            }
        });
        return sum;
