...
describe('#hashObject()', () => {
    ...
    describe('when success', () => {
        ...
        [
            ...
            {
                description: 'Hash empty recursive array',
                hashProperties: [
                    'someArray.[]', 'propA', 'propB'
                ],
                obj: {
...
describe('#sumBalances()', () => {
    ...
    const emptyBal = [];
    context('single balance', () => {
        it('returns the input', () => {
            expect(utils.sumBalances(posBal1)).to.deep.equal(posBal1);
        });
    });

    context('two balances', () => {
        const sum = [
            {
                symbol: 'ABC', 
                currency: {ct: '0x01', id: 0}, 
            ...
            }, 
            {
                symbol: 'ETH', 
                currency: {ct: '0x0000000000000000000000000000000000000000', id: '0'}, 
                decimalAmount: ethers.utils.formatUnits('10', 18), 
                amount: '10'
            },
            {
                symbol: 'ABC2', 
        ...
        it('returns the correct sum', () => {
            expect(utils.sumBalances(posBal1, posBal2)).to.deep.equal(sum);
        });

        it('returns the correct sum with an extra empty bal', () => {
            expect(utils.sumBalances(posBal1, emptyBal, posBal2)).to.deep.equal(sum);
        });
    });
});
