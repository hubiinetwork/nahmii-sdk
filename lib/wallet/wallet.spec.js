    ...
    describe(`Wallet with ${description}`, () => {
        ...
        context('a wallet with nahmii balance', () => {
            ...
            it('has a nahmii balance', async () => {
                let balance = await wallet.getAvaliableBalances();
                expect(balance).to.eql(balances);
            });
        ...
        context('a wallet with a base layer balance', () => {
            ...
            it('can fetch it\'s eth bal using the ethers getBalance method', async () => {
            ...
            it('has a base layer balance', async () => {
                stubbedProvider.getBaseLayerBalances
                    .withArgs(walletAddress)
                    .resolves(balances);
                const res = await wallet.getBaseLayerBalances();
                expect(res).to.equal(balances);
            });
        ...
        context('a wallet with a staged balance', () => {
            ...
            it('has a staged balance', async () => {
                stubbedProvider.getStagedBalances
                    .withArgs(walletAddress)
                    .resolves(balances);
                const res = await wallet.getStagedBalances();
                expect(res).to.equal(balances);
            });
        ...
        context('a wallet with a staging balance', () => {
            ...
            it('has a staging balance', async () => {
                stubbedProvider.getStagingBalances
                    .withArgs(walletAddress)
                    .resolves(balances);
                const res = await wallet.getStagingBalances();
                expect(res).to.equal(balances);
            });
        });
    });
