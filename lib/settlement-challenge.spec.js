...
describe('Settlement operations', () => {
    ...
    [true, null].forEach(t => {
        it(`can start payment challenge period for eth when #hasPaymentChallengeProposalExpired returns ${t}`, async () => {
            ...
            stubbedDriipSettlementChallengeContract.startChallengeFromPayment
                ...
                .resolves(fakeTx);
            sinon.stub(settlementChallenge, 'hasPaymentChallengeProposalExpired').resolves(t);
    ...
    it('can not start payment challenge period', (done) => {
        ...
        sinon.stub(settlementChallenge, 'hasPaymentChallengeProposalExpired').resolves(false);
    ...
    testTokens.forEach(t => {
        it(`can start payment challenge period for token ${t.symbol}`, async () => {
            ...
            stubbedDriipSettlementChallengeContract.startChallengeFromPayment
                .withArgs(receipt.toJSON(), stageAmount.toJSON().amount)
                .resolves(fakeTx);
            sinon.stub(settlementChallenge, 'hasPaymentChallengeProposalExpired').resolves(true);
            let tx = await settlementChallenge.startChallengeFromPayment(receipt, stageAmount, wallet);
            expect(tx).to.equal(fakeTx);
        });
    });
        ...
        challengeStatuses.forEach((t, i) => {
            it(`can correctly parse challenge status ${t}`, async () => {
                stubbedDriipSettlementChallengeContract.proposalStatus
                    ...
                    .resolves(i);
                const status = await settlementChallenge.getCurrentPaymentChallengeStatus(wallet.address, address0, address0id);
    ...
    describe('challenge nonce', () => {
        it('can correctly return nonce', async () => {
            ...
            stubbedDriipSettlementChallengeContract.proposalNonce
                .withArgs(wallet.address, address0, address0id)
                .resolves(nonce);
            const status = await settlementChallenge.getCurrentPaymentChallengeNonce(wallet.address, address0, address0id);
    ...
    describe('challenge expiration time', () => {
        it('can correctly return expiration time', async () => {
            ...
            stubbedDriipSettlementChallengeContract.proposalExpirationTime
                .withArgs(wallet.address, address0, address0id)
                .resolves(timeout);
            const _timeout = await settlementChallenge.getCurrentPaymentChallengeExpirationTime(wallet.address, address0, address0id);
    ...
    describe('challenge expiration check', () => {
        it('can correctly return if expired', async () => {
            const hasExpired = true;
            stubbedDriipSettlementChallengeContract.hasProposalExpired
                .withArgs(wallet.address, address0, address0id)
                .resolves(hasExpired);
            const _hasExpired = await settlementChallenge.hasPaymentChallengeProposalExpired(wallet.address, address0, address0id);
        ...
        it('should return null when exception thrown', async () => {
            stubbedDriipSettlementChallengeContract.hasProposalExpired
                .withArgs(wallet.address, address0, address0id)
                .throws('reverted');
            const _timeout = await settlementChallenge.hasPaymentChallengeProposalExpired(wallet.address, address0, address0id);
            expect(_timeout).to.equal(null);
        });
    });

    describe('challenge stage amount', () => {
        it('can correctly return proposal stage amount', async () => {
            ...
            const _stageAmount = await settlementChallenge.getCurrentPaymentChallengeStageAmount(wallet.address, address0, address0id);
        ...
        it('should return null when exception thrown', async () => {
            stubbedDriipSettlementChallengeContract.proposalStageAmount
                .withArgs(wallet.address, address0, address0id)
                .throws('reverted');
            const _hasExpired = await settlementChallenge.getCurrentPaymentChallengeStageAmount(wallet.address, address0, address0id);
