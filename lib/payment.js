...
const PAYMENT_REQUEST_HASHED_PROPERTIES = ['amount', 'currency', 'sender.addr', 'recipient.addr'];
...
class Payment {
     ...
     * @param {String|BigNumber} amount - Amount in base units (wei for ETH)
     * @param {Address} currency - Currency identifier for the payment, 0x0000000000000000000000000000000000000000 for ETH
     ...
     */
    sign(privateKey) {
        const h = hashObject(this.toJSON(), PAYMENT_REQUEST_HASHED_PROPERTIES);
    ...
    isSigned() {
        ...
        const h = hashObject(serializedPayment, PAYMENT_REQUEST_HASHED_PROPERTIES);
     ...
     * @returns {{amount: String, currency: String, sender: {addr: String}, recipient: {addr: String}, seals: {wallet: {hash: String, signature: String}}}}
     */
    toJSON() {
        const result = {
     ...
     * @param payload - A JSON object that can be de-serialized to a Payment instance
     * @returns {Payment}
     */
