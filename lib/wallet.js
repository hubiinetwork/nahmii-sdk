...
class Wallet extends ethers.Wallet {
    ...
    async startChallengeFromPayment(receipt, stageAmount, options = {}) {
        //if previous challenge phrase is in dispute, not allow to start new challenge
    ...
    async settleDriipAsPayment(receipt, options = {}) {
        //if current challenge phrase is in dispute or status is disqualified, not allow to settle
        const currentPhase = await this.getCurrentPaymentChallengePhase();
    ...
    /**
     * Without an amount of ETH or ERC20 tokens from nahmii to base layer.
