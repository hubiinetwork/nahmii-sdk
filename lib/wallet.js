...
class Wallet extends ethers.Wallet {
    ...
    async getAvaliableBalances() {
        ...
        const [nahmiiBalances, supportedTokens] = await Promise.all([
            this.provider.getAvaliableBalances(this.address),
    ...
    /**
     * Retrieves all nahmii deposited balances for the wallet.
     * @return {Promise} A promise that resolves into a balances object.
     */
    getDepositedBalances() {
        return this.provider.getDepositedBalances(this.address);
    }

    /**
     * Retrieves all nahmii settled balances for the wallet.
     * @return {Promise} A promise that resolves into a balances object.
     */
    getSettledBalances() {
        return this.provider.getSettledBalances(this.address);
    }
