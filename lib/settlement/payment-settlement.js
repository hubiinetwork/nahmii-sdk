...
class PaymentSettlement extends SettlementBase {
    ...
    static async load(address, ct, provider) {
        ...
        const [nonce, stageAmount] = Promise.all([
            await driipSettlementChallengeContract.getCurrentProposalNonce(address, ct, 0),
            await driipSettlementChallengeContract.getCurrentProposalStageAmount(address, ct, 0)
        ]);
        if (!nonce) 
            return null;
