...
const NullSettlementStateContract = require('./null-settlement-state-contract');
...
const _nullSettlementStateContract = new WeakMap();
...
class NullSettlement {
    ...
    constructor(provider) {
        ...
        _nullSettlementStateContract.set(this, new NullSettlementStateContract(provider));
    ...
    async getMaxNullNonce(address, ct, id) {
        try {
            const nullSettlementStateContract = _nullSettlementStateContract.get(this);
            const maxNullNonce = await nullSettlementStateContract.maxNonceByWalletAndCurrency(address, {ct, id});
            return maxNullNonce;
        }
        catch (err) {
            if (isRevertContractException(err)) 
    ...
    async checkSettleNull(address, ct, id) {
        try {
            const [expired, currentStatus, currentNonce, maxNullNonce] = await Promise.all([
