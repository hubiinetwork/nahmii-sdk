...
describe('ContinuousSettlement', () => {
    ...
    context('initiate from constructor', () => {
        ...
        it('should return an instance with no states sync from contracts', () => {
            const continuousSettlement = new ContinuousSettlement(stubbedWallet.address, currency.ct, ethers.utils.bigNumberify(1), stubbedProvider);
        ...
        context('determine instance properties from contract states', () => {
            ...
            },
            {
                ...
                it(name, async () => {
                    ...
                    const continuousSettlement = await ContinuousSettlement.load(wallet.address, currency.ct, stubbedProvider);
    ...
    context('#create()', () => {
        ...
        [{
            ...
            it(name, async () => {
                mockContractStates(contract, wallet, currency);
                try {
                    const continuousSettlement = await ContinuousSettlement.create(wallet.address, currency.ct, stageAmount, stubbedProvider);
                    if (continuousSettlement)
                        expect(continuousSettlement.toJSON()).to.deep.equal(expectedInstance);
                    else
                        expect(expectedInstance).to.equal(null);
    ...
    context('#checkForCreate()', () => {
        ...
        [{
            ...
            it(name, async () => {
                mockContractStates(contract, wallet, currency);
                const checks = await ContinuousSettlement.checkForCreate(wallet.address, currency.ct, stubbedProvider);
    ...
    context('#start()', () => {
        ...
        context('from #create()', () => {
            beforeEach(async () => {
                continuousSettlement = await ContinuousSettlement.create(stubbedWallet.address, currency.ct, stageAmount, stubbedProvider);
            });
            ...
            context('when starts successfully', () => {
                ...
                beforeEach(() => {
                    ...
                    stubbedNullSettlementChallengeContract
                        ...
                        .withArgs([
                            continuousSettlement.stageAmount,
                            continuousSettlement.currency,
                            0
                        ])
                        .returns(data);
                    stubbedWallet
                ...
                it('returns registered settlement', async () => {
                    const tx = await continuousSettlement.start(stubbedWallet, options);
            ...
            context('when throws exception', () => {
                ...
                it('should throw error and not re-sync', async () => {
                    try {
                        await continuousSettlement.start(stubbedWallet);
                    }
                    catch (e) {
        ...
        context('from #load()', () => {
            ...
            beforeEach(async () => {
                ...
                continuousSettlement = await ContinuousSettlement.load(stubbedWallet.address, currency.ct, stubbedProvider);
            });
            it('should throw when #isStarted=true', (done) => {
                continuousSettlement.start(stubbedWallet, options).catch(error => {
    ...
    context('#stage()', () => {
        ...
        context('when stages successfully', () => {
            ...
            it('returns transaction', async () => {
                const tx = await continuousSettlement.stage(stubbedWallet, options);
        ...
        context('throw exception', () => {
            context('when #settleNull() throws', () => {
                ...
                it('should throw error and not re-sync', (done) => {
                    continuousSettlement.stage(stubbedWallet).catch(e => {
                        expect(e.innerError.message).to.equal(error.message);
                        expect(continuousSettlement.toJSON()).to.deep.equal(newSettlementObject);
                        done();
                    });
            ...
            context('when the condictions does not allow', () => {
                ...
                it('should throw when #isStageable=false', (done) => {
                    continuousSettlement.stage(stubbedWallet, options).catch(e => {
