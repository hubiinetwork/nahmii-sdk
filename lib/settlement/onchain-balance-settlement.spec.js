...
describe('OnchainBalanceSettlement', () => {
    ...
    context('initiate from constructor', () => {
        ...
        it('should return an instance with no states sync from contracts', () => {
            ...
            const currencyAddress = EthereumAddress.from(currency.ct);
            const onchainBalanceSettlement = new OnchainBalanceSettlement(walletAddress, currencyAddress, ethers.utils.bigNumberify(1), stubbedProvider);
        ...
        context('determine instance properties from contract states', () => {
            ...
            },
            {
                ...
                it(name, async () => {
                    ...
                    const onchainBalanceSettlement = await OnchainBalanceSettlement.load(wallet.address, currency.ct, stubbedProvider);
    ...
    context('#create()', () => {
        ...
        [{
            ...
            it(name, async () => {
                mockContractStates(contract, wallet, currency);
                try {
                    const onchainBalanceSettlement = await OnchainBalanceSettlement.create(wallet.address, currency.ct, stageAmount, stubbedProvider);
                    if (onchainBalanceSettlement)
                        expect(onchainBalanceSettlement.toJSON()).to.deep.equal(expectedInstance);
                    else
                        expect(expectedInstance).to.equal(null);
    ...
    context('#checkForCreate()', () => {
        ...
        [{
            ...
            it(name, async () => {
                mockContractStates(contract, wallet, currency);
                const checks = await OnchainBalanceSettlement.checkForCreate(wallet.address, currency.ct, stubbedProvider);
    ...
    context('#start()', () => {
        ...
        context('from #create()', () => {
            beforeEach(async () => {
                onchainBalanceSettlement = await OnchainBalanceSettlement.create(stubbedWallet.address, currency.ct, stageAmount, stubbedProvider);
            });
                ...
                beforeEach(() => {
                    ...
                    stubbedNullSettlementChallengeContract
                        ...
                        .withArgs([
                            ...
                            onchainBalanceSettlement.currency.toString(),
                            0
                        ])
                        .returns(data);
                    stubbedWallet
        ...
        context('from #load()', () => {
            ...
            beforeEach(async () => {
                ...
                onchainBalanceSettlement = await OnchainBalanceSettlement.load(stubbedWallet.address, currency.ct, stubbedProvider);
            });
            ...
            context('when the condictions does not allow', () => {
                ...
                it('should throw when #isStageable=false', (done) => {
                    onchainBalanceSettlement.stage(stubbedWallet, options).catch(e => {
                        ...
                        expect(onchainBalanceSettlement.toJSON()).to.deep.equal(Object.assign({}, newSettlementObject, {isStageable: false, isCompleted: true}));
                        done();
                    });
                });
            });
