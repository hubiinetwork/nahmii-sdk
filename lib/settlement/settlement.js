...
class Settlement {
    ...
    async checkStartChallenge(stageMonetaryAmount, latestReceipt, walletAddress) {
        ...
        try {
            ...
            const balance = balances.find(bal => bal.currency.ct === currency.ct);
            const balanceBN = ethers.utils.bigNumberify(balance.amount);

            if (balanceBN.lt(stageAmountBN))
                throw new Error(`The maximum allowable stage balance is ${ethers.utils.formatUnits(balanceBN, tokenInfo.decimals)}`);
    ...
    async getSettleableChallenges(address, ct, id) {
        ...
        try {
            ...
            if (nonce) {
                ...
                const challengedReceipt = allWalletReceipts.find(r => {
                    if (caseInsensitiveCompare(address, r.sender.wallet) && r.sender.nonce === nonce.toNumber())
                        return true;
                    if (caseInsensitiveCompare(address, r.recipient.wallet) && r.recipient.nonce === nonce.toNumber())
                        return true;
                });
     ...
     * @param {OngoingChallenge} ongoingChallenge - The object stores the necessary parameters for stopping a settlement challenge.
