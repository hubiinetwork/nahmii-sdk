...
class DriipSettlement {
    ...
    async hasPaymentDriipSettled(nonce, address) {
        ...
        try {
            let historySettlement = await this.getSettlementByNonce(address, nonce);
    ...
    async checkStartChallengeFromPayment(receipt, address) {
        try {
            const {sender, recipient, currency} = receipt.toJSON();
            const nonce = caseInsensitiveCompare(sender.wallet, address) ? sender.nonce : recipient.nonce;
     ...
     * @param {Address} ct - The currency address.
     * @param {Integer} ct - The currency id.
     * @param [options]
     * @returns {Promise} A promise that resolves into transaction hash.
     */
