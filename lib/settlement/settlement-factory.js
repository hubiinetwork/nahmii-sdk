...
class SettlementFactory {
    ...
    async getAllSettlements(walletAddress, ct) {
        ...
        console.log(JSON.stringify(settlements, undefined, 2));
    ...
    async checkStartChallenge(stageMonetaryAmount, latestReceipt, walletAddress) {
        ...
        try {
            ...
            const ongoingSettlements = settlements.filter(settlement => settlement.isOngoing);
            const existContinuousSettlement = ongoingSettlements.find(settlement => settlement instanceof ContinuousSettlement);
            const existPaymentSettlement = ongoingSettlements.find(settlement => settlement instanceof PaymentSettlement);

            if (existContinuousSettlement && existContinuousSettlement.isOngoing) 
                return [];
            
            if (existPaymentSettlement === null || existContinuousSettlement.isCompleted) 
                allowedSettlements.push(PaymentSettlement);

            if (existContinuousSettlement === null || existContinuousSettlement.isCompleted) 
