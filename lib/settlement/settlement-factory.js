...
class SettlementFactory {
    ...
    async calculateRequiredSettlements(walletAddress, stageMonetaryAmount) {
        try {
            ...
            const settlementTypes = await this.getAllowedSettlementTypes(stageMonetaryAmount, latestReceipt, walletAddress);
            if (!settlementTypes.length)
                throw new Error('No allowed settlement available to start with.');

            if (settlementTypes.length === 1) {
                const Settlement = settlementTypes[0];
                const newSettlement = await Settlement.create(walletAddress, currency.ct, stageAmount, this.provider);
            ...
            for (const Settlement of settlementTypes) {
                if (Settlement.name === PaymentSettlement.name) {
                    const latestReceiptBalance = determineBalanceFromReceipt(latestReceipt, walletAddress);
                    if (ethers.utils.bigNumberify(remainingIntendedAmount).gt(latestReceiptBalance)) {
                        const paymentSettlement = await PaymentSettlement.create(walletAddress, currency.ct, latestReceiptBalance, this.provider);
                        newSettlements.push(paymentSettlement);
                        remainingIntendedAmount = remainingIntendedAmount.sub(latestReceiptBalance);
                    }
                    else {
                        const paymentSettlement = await PaymentSettlement.create(walletAddress, currency.ct, remainingIntendedAmount, this.provider);
                        newSettlements.push(paymentSettlement);
                        break;
                    }
                }
                else {
                    const newSettlement = await Settlement.create(walletAddress, currency.ct, remainingIntendedAmount, this.provider);
     ...
     */
    async getAllowedSettlementTypes(stageMonetaryAmount, latestReceipt, walletAddress) {
        ...
        try {
            ...
            const settlements = await this.getAllSettlements(walletAddress, currency.ct);
            const existSettlements = settlements.filter(settlement => settlement !== null);
            const existContinuousSettlement = existSettlements.find(settlement => settlement instanceof ContinuousSettlement);
            const existPaymentSettlement = existSettlements.find(settlement => settlement instanceof PaymentSettlement);
            const hasOverlappingContinuousSettlement = existContinuousSettlement && !existContinuousSettlement.isCompleted;
            
            if (latestReceipt) {
                if (!existPaymentSettlement || 
                    (existPaymentSettlement.isCompleted && JSON.stringify(existPaymentSettlement.receipt) !== JSON.stringify(latestReceipt))) 
                    allowedSettlements.push(PaymentSettlement);
            }

            if (hasOverlappingContinuousSettlement) 
                return [];
            else 
                allowedSettlements.push(ContinuousSettlement);
