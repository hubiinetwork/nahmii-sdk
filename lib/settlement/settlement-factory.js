...
class SettlementFactory {
    ...
    async getAllSettlements(walletAddress, ct) {
        const settlements = await Promise.all([
            ...
            OnchainBalanceSettlement.load(walletAddress, ct, this.provider)
        ]);
...
async function validateBalanceAvailable(walletAddress, ct, intendedStageAmount) {
    ...
    const balance = balances.find(bal => EthereumAddress.from(bal.currency.ct).isEqual(EthereumAddress.from(ct)));
...
async function getAllowedSettlementTypes(walletAddress, ct) {
    ...
    const onchainBalanceSettlementCheck = await OnchainBalanceSettlement.checkForCreate(walletAddress, ct, provider);
