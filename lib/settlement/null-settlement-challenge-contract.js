...
class NullSettlementChallengeContract extends NahmiiContract {
    ...
    async hasCurrentProposalExpired(address, ct, id) {
        try {
            return await this.hasProposalExpired(address, ct, id);
        }
        catch (err) {
    ...
    async hasCurrentProposalTerminated(address, ct, id) {
        try {
            return await this.hasProposalTerminated(address, ct, id);
        }
        catch (err) {
            if (isRevertContractException(err))
                return null;
    ...
    async getCurrentProposalExpirationTime(address, ct, id) {
        try {
            return await this.proposalExpirationTime(address, ct, id);
        }
        catch (err) {
            if (isRevertContractException(err))
                return null;
    ...
    async getCurrentProposalStageAmount(address, ct, id) {
        try {
            return await this.proposalStageAmount(address, ct, id);
        }
        catch (err) {
            if (isRevertContractException(err))
                return null;
    ...
    async getCurrentProposalStatus(address, ct, id) {
        ...
        try {
            const statusIndex = await this.proposalStatus(address, ct, id);
