...
class StubbedEthersWallet extends ethers.Wallet {
    constructor(signer, provider) {
        super(signer, provider);
    }
}
StubbedEthersWallet.prototype.getBalance = sinon.stub();
StubbedEthersWallet.prototype.getTransactionCount = sinon.stub();
StubbedEthersWallet.prototype.sendTransaction = sinon.stub();

const stubbedEthers = {
    ...ethers,
    Wallet: StubbedEthersWallet
};
...
[
    ...
    describe(`Wallet with ${description}`, () => {
        ...
        afterEach(() => {
            ...
            stubbedEthers.Wallet.prototype.getBalance.reset();
            stubbedEthers.Wallet.prototype.getTransactionCount.reset();
            stubbedEthers.Wallet.prototype.sendTransaction.reset();
        });
        ...
        context('a wallet with no base layer balance', () => {
            let walletBal;
            beforeEach(() => {
                walletBal = ethers.utils.bigNumberify('0');
                stubbedEthers.Wallet.prototype.getBalance
                    .withArgs()
                    .resolves(walletBal);
            });
