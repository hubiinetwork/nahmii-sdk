...
class NahmiiProvider extends ethers.providers.JsonRpcProvider {
    ...
    /**
     * Retrieves all settled balances for the specified wallet address.
     * @param {Address} address
     * @return {Promise} A promise that resolves into a balances object.
     */
    async getSettledBalances(address) {
        const clientFund = _clientFund.get(this);
        await this.getApiAccessToken();

        const currenciesArr = await this.getAllSupportedCurrencies();
        const balancesArr = await Promise.all(currenciesArr.map(asset => clientFund.settledBalance(address, asset.currency, 0)));

        const settledBalances = {};
        balancesArr.forEach((bal, i) => {
            if (bal.gt('0')) {
                const currency = currenciesArr[i];
                settledBalances[currency.symbol] = ethers.utils.formatUnits(bal, currency.decimals);
            }
        });
    ...
    /**
     * Retrieves all deposited balances for the specified wallet address.
     * @param {Address} address
     * @return {Promise} A promise that resolves into a balances object.
     */
    async getDepositedBalances(address) {
        const clientFund = _clientFund.get(this);
        await this.getApiAccessToken();

        const currenciesArr = await this.getAllSupportedCurrencies();
        const balancesArr = await Promise.all(currenciesArr.map(asset => clientFund.depositedBalance(address, asset.currency, 0)));

        const depositedBalances = {};
        balancesArr.forEach((bal, i) => {
            if (bal.gt('0')) {
                const currency = currenciesArr[i];
                depositedBalances[currency.symbol] = ethers.utils.formatUnits(bal, currency.decimals);
            }
        });

        return depositedBalances;
    }
    ...
    async getBaseLayerBalances(address) {
        await this.getApiAccessToken();
        const balancesArr = await _nahmii.get(this).get(`ethereum/wallets/${address}/balances`);
        const currenciesArr = await this.getAllSupportedCurrencies();
