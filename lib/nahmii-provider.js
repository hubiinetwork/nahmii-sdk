...
class NahmiiProvider extends ethers.providers.JsonRpcProvider {
    ...
    async getStagingBalances(address) {
        ...
        const currenciesArr = await this.getAllSupportedCurrencies();
        const balancesArr = await Promise.all(currenciesArr.map(asset => driipSettlementChallenge.proposalStageAmount(address, {ct: asset.currency, id: ethers.utils.bigNumberify(0) })));

        const stagingBalances = {};
        balancesArr.forEach((bal, i) => {
            if (bal.gt('0')) {
                const currency = currenciesArr[i];
                stagingBalances[currency.symbol] = ethers.utils.formatUnits(bal, currency.decimals);
            }
        });

        return stagingBalances;
    }
