...
describe('Null settlement operations', () => {
    ...
    it('can not start challenge if wallet is locked', (done) => {
        ...
        nullSettlement.startChallenge(wallet, stageAmount).catch(err => {
            ...
            expect(err.reasons[0].message).to.match(/wallet.*locked/i);
            done();
        });
    });
    ...
    it('can not settle null if wallet is locked', (done) => {
        ...
        nullSettlement.settleNull(currency, 0).catch(err => {
            ...
            expect(err.reasons.length).to.equal(1);
            expect(err.reasons[0].message).to.match(/locked/i);
            done();
        });
    });
    ...
    describe('#checkStartChallenge', () => {
        ...
        it('should be invalid to start new challenge', async () => {
            const walletLockError = new Error('Wallet is locked!');
            const notExpiredError = new Error('Current challenge proposal has not expired!');
            const expectedResult = {
                valid: false, 
                reasons: [walletLockError, notExpiredError]
            };

            stubbedNullSettlementChallengeContract.isLockedWallet
    ...
    describe('#checkSettleNull', () => {
        ...
        it('should be invalid to start new challenge', async () => {
            ...
            const expectedResult = {valid: false, reasons: [walletLockError, notExpiredError, statusError, replayError]};
