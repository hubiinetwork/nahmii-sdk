 ...
 * const expired = await nullSettlement.hasCurrentProposalExpired(walletAddress, currencyContract, currencyId)
 */
class NullSettlement {
    /**
     * Constructor
     ...
     * @example
     * let locked = await nullSettlement.isLocked(address);
     */
    ...
    async hasCurrentProposalExpired(address, ct, id) {
        const challengeContract = _nullSettlementChallengeContract.get(this);
        try {
            return await challengeContract.hasCurrentProposalExpired(address, ct, id);
        } catch (err) {
            return null;
        }
    }
    ...
    async settleNull(wallet, ct, id, options = {}) {
        const locked = await this.isWalletLocked(wallet.address);
        const expired = await this.hasCurrentProposalExpired(wallet.address, ct, id);
        const currentStatus = await this.getCurrentProposalStatus(wallet.address, ct, id);
        const currentNonce = await this.getCurrentProposalNonce(wallet.address, ct, id);
        const maxNullNonce = await this.getMaxNullNonce(wallet.address, ct, id);
    ...
    async startChallenge(wallet, stageAmount, options = {}) {
        const stageAmountJSON = stageAmount.toJSON();
        const locked = await this.isWalletLocked(wallet.address);
        const expired = await this.hasCurrentProposalExpired(wallet.address, stageAmountJSON.currency.ct, stageAmountJSON.currency.id);
        ...
        return await challengeContract.startChallenge(stageAmountJSON.amount, stageAmountJSON.currency.ct, stageAmountJSON.currency.id, options);
    }
}
