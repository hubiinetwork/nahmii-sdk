...
const stubbedBatchRpcRequest = sinon.stub();
...
function proxyquireProvider() {
    return proxyquire('./nahmii-provider', {
        './batch-rpc-request': stubbedBatchRpcRequest,
...
describe('Nahmii Provider', () => {
    ...
    function validateExpectedBehaviorOfProvider() {
        ...
        it('can retrieve the base layer balance for a wallet', async () => {
            ...
            const expectedBalances = [ 
                { 
                    ...
                    balance: '137104618244215607775',
                    amount: '137104618244215607775',
                    symbol: 'ETH',
                    decimalAmount: '137.104618244215607775'
                },
                { 
                    address: walletAddr,
                    currency: {
                    ...
                    balance: '1097796710986654323',
                    amount: '1097796710986654323', 
                    decimalAmount: '1.097796710986654323',
                    symbol: 'BOKKY'
                } 
            ];
        ...
        [
            ...
            describe('contract staged balance checks', () => {
                const expectedTokens = [{ currency: '0x01', symbol: 'ABC', decimals: 12 }];
                const balances = {ABC: ethers.utils.bigNumberify('1001'), ETH: ethers.utils.bigNumberify('5')};
                const expected = [
                    {
                        symbol: 'ABC', 
                        currency: {ct: '0x01', id: '0'}, 
                        decimalAmount: ethers.utils.formatUnits('1001', 12), 
                        amount: '1001'
                    }, 
                    {
                        symbol: 'ETH', 
                        currency: {ct: '0x0000000000000000000000000000000000000000', id: '0'}, 
                        decimalAmount: ethers.utils.formatUnits('5', 18), 
                        amount: '5'
                    }
                ];
                ...
                it('can retrieve the staged balances for an address', async () => {
                    stubbedClientFund.stagedBalance
                        ...
                        .resolves(balances['ABC']);
                    stubbedClientFund.stagedBalance
                        .withArgs(walletAddr, '0x0000000000000000000000000000000000000000', 0)
                        .resolves(balances['ETH']);
                    const result = await provider.getStagedBalances(walletAddr);
                    expect(result).to.deep.equal(expected);
                });
            });

            describe('contract staging balance checks', () => {
                const expectedTokens = [
                    { currency: '0x01', symbol: 'ABC1', decimals: 12 }, 
                    { currency: '0x02', symbol: 'ABC2', decimals: 12 }, 
                    { currency: '0x03', symbol: 'ABC3', decimals: 12 }
                ];
                ...
                it('can retrieve the staging balances for an address', async () => {
                    const balances = {ABC2: ethers.utils.bigNumberify('1001'), ETH: ethers.utils.bigNumberify('5')};
                    const expected = [
                        {
                            symbol: 'ABC2', 
                            currency: {ct: '0x02', id: '0'}, 
                            decimalAmount: ethers.utils.formatUnits('1001', 12), 
                            amount: '1001'
                        }, 
                        {
                    ...
                    stubbedBatchRpcRequest
                        .resolves([0, 1, 0, 1]);
                    stubbedDriipSettlementChallenge.proposalStageAmount
                        .withArgs(walletAddr, '0x02', 0)
                        .resolves(balances['ABC2']);
                    stubbedDriipSettlementChallenge.proposalStageAmount
                        .withArgs(walletAddr, '0x0000000000000000000000000000000000000000', 0)
                        .resolves(balances['ETH']);
