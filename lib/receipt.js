...
class Receipt {
    ...
    isSigned() {
        ...
        let serializedReceipt = this.toJSON();
     ...
     * @param payload - A JSON object that can be de-serialized to a Receipt instance
     * @returns {Receipt}
     */
    static from(provider, payload) {
        const p = Payment.from(provider, payload);
        const r = new Receipt(provider, p);

        _nonce.set(r, payload.nonce);

        if (payload.sender) {
            _senderNonce.set(r, payload.sender.nonce);
            if (payload.sender.balances) {
                _senderCurrentBalance.set(r, payload.sender.balances.current);
                _senderPreviousBalance.set(r, payload.sender.balances.previous);
            }
            if (payload.sender.fees) {
                _senderSingleFee.set(r, MonetaryAmount.from(payload.sender.fees.single));
                if (payload.sender.fees.net) {
                    const netFees = payload.sender.fees.net.map(f => MonetaryAmount.from(f));
        ...
        if (payload.recipient) {
            _recipientNonce.set(r, payload.recipient.nonce);
            if (payload.recipient.balances) {
                _recipientCurrentBalance.set(r, payload.recipient.balances.current);
                _recipientPreviousBalance.set(r, payload.recipient.balances.previous);
            }
            if (payload.recipient.fees && payload.recipient.fees.net) {
                const netFees = payload.recipient.fees.net.map(f => MonetaryAmount.from(f));
                _recipientNetFees.set(r, netFees);
            }
        }

        if (payload.transfers) {
            _netTransfer.set(r, payload.transfers.net);
            _singleTransfer.set(r, payload.transfers.single);
        }

        if (payload.seals && payload.seals.exchange) {
            _hash.set(r, payload.seals.exchange.hash);
            _signature.set(r, payload.seals.exchange.signature);
        }
        return r;
    }
}
