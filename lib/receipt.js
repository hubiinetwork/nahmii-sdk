...
class Receipt {
    ...
    toJSON() {
        ...
        if (senderTotalFees && senderTotalFees.length)
            result.sender.fees.total = senderTotalFees.map(f => f.toJSON());
        ...
        if (recipientTotalFees && recipientTotalFees.length) {
            result.recipient.fees = {
                total: recipientTotalFees.map(f => f.toJSON())
            };
        }
    ...
    static from(provider, json) {
        ...
        if (json.sender) {
            ...
            if (json.sender.fees) {
                ...
                if (json.sender.fees.total) {
                    const totalFees = json.sender.fees.total.map(f => MonetaryAmount.from(f));
        ...
        if (json.recipient) {
            ...
            if (json.recipient.fees && json.recipient.fees.total) {
                const totalFees = json.recipient.fees.total.map(f => MonetaryAmount.from(f));
...
function hashReceipt(serializedReceipt) {
    ...
    // as uint256 values
    const receiptCopy = cloneDeep(serializedReceipt);
