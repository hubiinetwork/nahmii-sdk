...
class Settlement {
    ...
    async getLatestReceiptForSettlement(address, ct) {
        ...
        const filteredReceipts = receipts.filter(r => r.currency.ct === ct);
    ...
    async startChallenge(stageMonetaryAmount, wallet, options = {}) {
        ...
        const latestReceipt = await this.getLatestReceiptForSettlement(wallet.address, currency.ct);
        ...
        for (let requiredChallenge of requiredChallenges) {
            ...
            if (type === 'null')
                currentTx = await nullSettlement.startChallenge(wallet, stageMonetaryAmount, options);

            if (type === 'payment-driip')
                currentTx = await driipSettlement.startChallengeFromPayment(Receipt.from(receipt, wallet), stageMonetaryAmount, wallet, options);

            const txReceipt = await provider.getTransactionConfirmation(currentTx.hash);
    ...
    async settle(ct, id, wallet, options = {}) {
        ...
        for (let settleableChallenge of settleableChallenges) {
            ...
            if (type === 'null')
                currentTx = await nullSettlement.settleNull(wallet, ct, id, options);

            if (type === 'payment-driip')
                currentTx = await driipSettlement.settleDriipAsPayment(Receipt.from(receipt, wallet), wallet, options);
