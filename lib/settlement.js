...
class Settlement {
    ...
    async checkStartChallengeFromPayment(stageMonetaryAmount, latestReceipt, walletAddress) {
        const driipSettlement = _driipSettlement.get(this);
        const nullSettlement = _nullSettlement.get(this);

        // const receipts = await provider.getWalletReceipts(walletAddress, null, 1000);
        try {
            if (latestReceipt) 
    ...
    async startChallenge(stageMonetaryAmount, wallet, options = {}) {
        ...
        const txs = [];
        for (let i = 0; i < requiredChallenges.length; i++) {
            const {type, receipt, stageMonetaryAmount} = requiredChallenges[i];
            let tx;
            if (type === 'null') 
    ...
    async settle(ct, id, wallet, options = {}) {
        ...
        const txs = [];
        for (let i = 0; i < settleableChallenges.length; i++) {
            const {type, receipt} = settleableChallenges[i];
            let tx;
            if (type === 'null') 
